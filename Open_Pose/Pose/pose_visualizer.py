    ...
    def draw_pose_rgb(npimg, humans, imgcopy=False):
        ...
        for human in humans:
            ...
            for i in range(CocoPart.Background.value):
                ...
                center_x = body_part.x * image_w + 0.5#四舍五入
                center_y = body_part.y * image_h + 0.5
                center = (int(center_x), int(center_y))
                centers[i] = center#获取各关节点坐标

                #record_joints_norm += [round(center_x/1280, 2), round(center_y/720, 2)]#720P下的归一化，保留两位小数
                record_joints_norm += [round(center_x,2),round(center_y,2)]

                xs.append(center[0])#这是在原图尺寸下的二维坐标
                ys.append(center[1])
                # 绘制关节点
                cv.circle(npimg, center, 3, CocoColors[i], thickness=TfPoseVisualizer.Thickness_ratio * 2,
                          lineType=8, shift=0)
                #cv.putText(npimg,str(i),(center[0],center[1]-8),cv.FONT_HERSHEY_COMPLEX,1,(0,0,255),3)
            # 将属于同一人的关节点按照各个部位相连
            ...
            # 记录coco的1号点作为xcenter
            if 1 in centers:
                xcenter.append(centers[1][0])#记录每个人的同一个关节点的坐标
